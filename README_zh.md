...
| 模式 | 说明 |
|------|------|
| 🔄 **轮询** | 按分组独立轮转渠道；健康分仅用于降级明显异常的渠道，不会让单个成功渠道长期霸占首位 |
| 🎲 **随机** | 每次随机选择一个可用渠道；健康分仅用于后移异常渠道，保留随机主语义 |
| 🛡️ **故障转移** | 优先使用高优先级渠道，仅当其故障或健康分过低时才后移；同优先级内会低频探索长期未采样的健康渠道或健康 key |
| ⚖️ **加权分配** | 依据权重和近期成功率排序；同时会低频探索长期未采样的健康渠道或健康 key，避免后排候选长期没有真实样本 |

> 💡 **示例**：创建分组名称为 `gpt-4o`，将多个供应商的 GPT-4o 渠道加入该分组，即可通过统一的 `model: gpt-4o` 访问所有渠道。

> 🩺 **健康分说明**：健康分现在默认只对异常渠道或异常 key 施加惩罚，不再因为连续成功而持续把某个候选抬到最前。若开启会话保持（`session_keep_time`），粘性命中仍优先于探索逻辑。低频探索间隔可在设置页配置，默认每 6 次请求触发一次；同渠道多 key 的补采样可通过设置页单独开关控制。
